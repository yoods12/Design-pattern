...
public:
    ...
    // 생성자를 미리 지정해줘야함
    test(int v1, string v2, float v3) { 
        var1 = v1;
        var2 = v2;
        var3 = v3;
    }
    void show() {// 해당 클래스가 가지는 원소들
        cout << "int_var : " << var1 << endl;
        cout << "string_var : " << var2 << endl;
        cout << "float_var : " << var3 << endl;
    }
...
int main()
{
    ...
    test t = build.int_var(10) // 원래는 test t = build.int_var(10).string_var("abc") .float_var(10.5).build(); 이런 식이나 가독성을 위해 들여쓰기를 했다
        .string_var("abc") 
        .float_var(10.5)
        .build();
